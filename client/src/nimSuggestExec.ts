...
export async function initNimSuggest(): Promise<Object> {
    prepareConfig();
    ...
    if (fs.existsSync(nimSuggestNewPath)) {
        ...
        if (versionArgs && versionArgs.length === 2) {
            ...
            return Promise.reject(`Can't get Nimsuggest version `)
        }
        ...
        return Promise.reject(`${nimSuggestNewPath} not existed!`)
    }
...
export function getCallMethodPlayload(method: string, ...parameter: sexp.SExp[]): string {
...
export async function execNimSuggest(suggestType: NimSuggestType, filename: string,
    ...
    if (!nimSuggestExec) {
        return Promise.reject('nimsuggest not found!');
    }
    ...
    let params = [suggestCmd, { kind: 'string', str: normalizedFilename }, { kind: 'number', n: line }, { kind: 'number', n: column }, { kind: 'string', str: dirtyFile }];
    let playload = getCallMethodPlayload.apply(null, params);
    console.log(playload)
    // return client.onReady().then( () => {
        return client.sendRequest(playload).then(ret => {
            console.log(ret,233)
            // trace(desc.process.pid, projectFile, suggestCmd + ' ' + normalizedFilename + ':' + line + ':' + column);
        // let ret = await desc.rpc.callMethod(suggestCmd,);
